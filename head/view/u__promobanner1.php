		...
		if($media_size<(1000*1000))
			{
				...
				if(move_uploaded_file($tmp, $media_path.$promobanner1_image)) {
					...
					//Insert query
					//Insert query
					$arrFields=array('`promobanner_title`','`display_order`','`display_size`','`promobanner_image`','`banner_link`','`banner_offertype`','`banner_link_type`','`link_open_type`','`status`');
					$arrValues=array("$promobanner_title","$displayorder","$display_size","$promobanner1_image","$banner_link","$banner_offertype","$banner_link_type","$link_open_type","$promobannerstatus");
